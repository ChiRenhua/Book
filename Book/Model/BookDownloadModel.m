...
- (void)getBookFileListWithBookInfo:(Book *)bookInfo {
    NSString *urlString = [NSString stringWithFormat:@"%@/bookmgyun/servlet/getPdfInfo.serv?username=%@&sessionid=%@&bookid=%@", [Utils getServerAddress], [[UserInfoModel sharedInstance] getUserName], [[UserInfoModel sharedInstance] getUserSessionid], bookInfo.bookID];
    NSString *url = [Utils UTF8URL:urlString];
    [self getBookListWithURL:url];
}

- (void)getBookImageListWithBookInfo:(Book *)bookInfo {
    NSString *urlString = [NSString stringWithFormat:@"%@/bookmgyun/servlet/getPicInfo.serv?username=%@&sessionid=%@&bookid=%@", [Utils getServerAddress], [[UserInfoModel sharedInstance] getUserName], [[UserInfoModel sharedInstance] getUserSessionid], bookInfo.bookID];
    NSString *url = [Utils UTF8URL:urlString];
    [self getBookListWithURL:url];
}
